...
@Component({
  ...
  template: `
    ...
    @if (planError()) {
      <div class="plan-error"><mat-icon>error_outline</mat-icon><span>{{ planError() }}</span><button type="button" (click)="planError.set(null)">Kapat</button></div>
    }
...
export class QueriesComponent implements OnInit {
  ...
  readonly planCopied = signal(false);
  ...
  async copyPlan(plan: QueryPlan): Promise<void> {
    await navigator.clipboard.writeText(plan.planXml);
    this.planCopied.set(true);
    setTimeout(() => this.planCopied.set(false), 1600);
  }

  downloadPlan(plan: QueryPlan): void {
    const blob = new Blob([plan.planXml], { type: 'application/xml;charset=utf-8' });
    const url = URL.createObjectURL(blob);
    const anchor = document.createElement('a');
    anchor.href = url;
    anchor.download = `query-${plan.queryId}-plan-${plan.planId}.sqlplan`;
